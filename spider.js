...
class Spider {
    ...
    updateLegNatural(leg) {
        ...
        const amplitudeVariation = 1 + (leg.index % 3) * 0.2;
        ...
        if (leg.isLifted) {
            ...
            const liftVariation = 1 + (leg.index % 2) * 0.1;
            const liftHeight = Math.sin(easeT * Math.PI) * 8 * liftVariation;
            ...
            if (dist > 18) {
                ...
                const adjustSpeed = 0.03 + (leg.index % 3) * 0.008;
